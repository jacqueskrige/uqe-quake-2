...
void GL_TextureMode( /*char *string*/ )
{
	...
	{
		ri.Con_Printf (PRINT_ALL, "unknown texture mode\n");
		ri.Cvar_Set ("gl_texturemode", "2");
	...
	if (gl_config.ext_anisotropic == true)
	{
		AnisotropyModes = (int)floor((log(gl_config.max_anisotropic)/log(2.0f)) + 0.5f);

		if (gl_texturemode->value > (2.0f + (float)AnisotropyModes))
		{
			ri.Con_Printf (PRINT_ALL, "unknown texture mode\n");
			ri.Cvar_SetValue ("gl_texturemode", (2.0f + (float)AnisotropyModes));
	...
	for (i=0, glt=gltextures ; i<numgltextures ; i++, glt++)
	{
		if (glt->type != it_pic && glt->type != it_sky )
		{
			...
			// jkrige - anisotropic filtering
			if (gl_texturemode->value > 2.0f && gl_config.ext_anisotropic == true)
				qglTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, pow(2.0f, gl_texturemode->value - 2.0f));
			else
...
// jkrige - palette removal - end

int		upload_width, upload_height;
//qboolean uploaded_paletted; // jkrige - palette removal
...
qboolean GL_Upload32 (unsigned *data, int width, int height,  qboolean mipmap)
{
	...
	//uploaded_paletted = false; // jkrige - palette removal
	...
	if (gl_config.ext_nonpow2 == true /*&& !mipmap*/)
	{
		scaled_width = width;
	...
	// jkrige - texture size
	if (scaled_width > gl_config.max_texturesize)
		scaled_width = gl_config.max_texturesize;
	if (scaled_height > gl_config.max_texturesize)
		scaled_height = gl_config.max_texturesize;
...
static unsigned	trans[2048 * 2048]; // jkrige - doubled textures

qboolean GL_Upload8 (byte *data, int width, int height,  qboolean mipmap, qboolean is_sky )
{
	//unsigned	trans[512*256]; // jkrige - doubled textures
	int			i, s;
	int			p;
	...
	else*/ // jkrige - palette removal
	{
		for (i=0 ; i<s ; i++)
		{
			...
			{	// transparent, so scan around for another color
				...
				if (i > width && dtdata[i-width] != 255)
					p = dtdata[i-width];
				else if (i < s-width && dtdata[i+width] != 255)
					p = dtdata[i+width];
				else if (i > 0 && dtdata[i-1] != 255)
					p = dtdata[i-1];
				else if (i < s-1 && dtdata[i+1] != 255)
					p = dtdata[i+1];
				else
					p = 0;
				// copy rgb components
...
nonscrap:
		...
		image->paletted = false; //uploaded_paletted; // jkrige - palette removal
		image->sl = 0;
		image->sh = 1;
		image->tl = 0;
		image->th = 1;
