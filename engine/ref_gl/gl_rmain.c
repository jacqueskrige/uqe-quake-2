...
// jkrige - transparent texture animation - begin
int			r_worldframe;
// jkrige - transparent texture animation - end
...
cvar_t	*gl_wireframe; // jkrige - wireframe
...
void GL_DrawParticles( int num_particles, const particle_t particles[], const unsigned colortable[768] )
{
	...
	// jkrige - wireframe - begin
	if (gl_wireframe->value)
		qglPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
	// jkrige - wireframe - end
...
// current viewcluster
	...
	if ( r_newrefdef.rdflags & RDF_NOWORLDMODEL )
	{
		...
		// jkrige - clear color changed to black
...
//	yfov = 2*atan((float)r_newrefdef.height/r_newrefdef.width)*180/M_PI;
	...
	MYgluPerspective (r_newrefdef.fov_y,  screenaspect,  4,  6144);
...
void R_Register( void )
{
	...
	gl_mode = ri.Cvar_Get( "gl_mode", "2", CVAR_ARCHIVE );
	...
	gl_clear = ri.Cvar_Get("gl_clear", "1", CVAR_ARCHIVE);
	// jkrige - changed gl_clear default value
	...
	gl_texturemode = ri.Cvar_Get( "gl_texturemode", "0", CVAR_ARCHIVE );
	//gl_texturemode = ri.Cvar_Get( "gl_texturemode", "GL_LINEAR_MIPMAP_NEAREST", CVAR_ARCHIVE );
	// jkrige - anisotropic filtering

	// jkrige - wireframe - begin
	gl_wireframe = ri.Cvar_Get("gl_wireframe", "0", 0);
	// jkrige - wireframe - end
	...
	vid_gamma = ri.Cvar_Get("vid_gamma", "2.2", CVAR_ARCHIVE);
	// jkrige - gamma
...
// jkrige - anisotropic filtering
int Get_MaxAnisotropic (void)
{
	return (int)floor((log(gl_config.max_anisotropic)/log(2.0f)) + 0.5f);
}
// jkrige - anisotropic filtering
...
{	
	...
	*/
	...
	// jkrige - remove GL_EXTENSIONS listing - begin
	//ri.Con_Printf (PRINT_ALL, "GL_EXTENSIONS: %s\n", gl_config.extensions_string );
	// jkrige - remove GL_EXTENSIONS listing - end

	// jkrige - texture size
	qglGetIntegerv(GL_MAX_TEXTURE_SIZE, &gl_config.max_texturesize);
	ri.Con_Printf (PRINT_DEVELOPER, "GL_MAX_TEXTURE_SIZE: %i\n", gl_config.max_texturesize );
	// jkrige - texture size
	...
	else if (strstr(vendor_buffer, "ati"))
	{
		...
		if (strstr(vendor_buffer, "radeon"))
			gl_config.renderer |= GL_RENDERER_RADEON;
	}
	...
	*/
	...
	if ( strstr( gl_config.extensions_string, "GL_ARB_multitexture" ) )
	{
		...
		{
			ri.Con_Printf( PRINT_ALL, "GL_ARB_multitexture\n" );
	...
	{
		ri.Con_Printf( PRINT_ALL, "GL_ARB_texture_non_power_of_two\n" );
		gl_config.ext_nonpow2 = true;
	}
	else
	{
		gl_config.ext_nonpow2 = false;
	}
	// jkrige - non power of two
	...
	{
		ri.Con_Printf( PRINT_ALL, "GL_EXT_compiled_vertex_array\n" );
	...
	if ( strstr( gl_config.extensions_string, "GL_EXT_point_parameters" ) )
	{
		...
		{
			ri.Con_Printf( PRINT_ALL, "GL_EXT_point_parameters\n" );
	...
	{
		ri.Con_Printf( PRINT_ALL, "GL_EXT_texture_filter_anisotropic\n" );
		qglGetFloatv(GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT, &gl_config.max_anisotropic);
		gl_config.ext_anisotropic = true;
	}
	else
	{
		gl_config.ext_anisotropic = false;
	}
	// jkrige - anisotropic filtering
	...
	{
		ri.Con_Printf( PRINT_ALL, "WGL_EXT_swap_control\n" );
...
void R_SetPalette ( const unsigned char *palette)
{
	...
	qglClearColor (0, 0, 0, 0);
	qglClear (GL_COLOR_BUFFER_BIT);
	//qglClearColor (1,0, 0.5 , 0.5); // jkrige - clear color changed to black
}
...
refexport_t GetRefAPI (refimport_t rimp )
{
	...
	// jkrige - anisotropic filtering
	re.GetMaxAnisotropic = Get_MaxAnisotropic;
	re.RunTextureMode = GL_RunTextureMode;
	// jkrige - anisotropic filtering
