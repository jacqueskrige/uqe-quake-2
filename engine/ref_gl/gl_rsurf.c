...
void R_RenderBrushPoly (msurface_t *fa)
{
	...
	// jkrige - transparent texture animation
	image = R_TextureAnimation (fa);
		...
		if (((psurf->flags & SURF_PLANEBACK) && (dot < -BACKFACE_EPSILON)) || (!(psurf->flags & SURF_PLANEBACK) && (dot > BACKFACE_EPSILON)))
		{
			...
			{	// add to the translucent chain
				...
				if (!duplicate) // Don't allow surface to be added twice (fixes hang)
				{
					...
					psurf->entity = currententity; // jkrige - transparent texture animation
				}
			...
			else if ( qglMTexCoord2fSGIS && !( psurf->flags & SURF_DRAWTURB ) )
			{
				...
				if (!gl_wireframe->value && !r_fullbright->value)
				{
					GL_RenderLightmappedPoly(psurf);
				}
				else
