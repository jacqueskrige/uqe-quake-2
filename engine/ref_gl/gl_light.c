	...
	for (lnum=0 ; lnum<r_newrefdef.num_dlights ; lnum++)
	{
		...
		for (t = 0, ftacc = 0 ; t<tmax ; t++, ftacc += 16)
		{
			...
			for ( s=0, fsacc = 0 ; s<smax ; s++, fsacc += 16, pfBL += 3)
			{
				...
				if ( fdist < fminlight )
				{
					// jkrige - improved dynamic light falloff
					pfBL[0] += ( fminlight /*frad*/ - fdist ) * dl->color[0];
					pfBL[1] += ( fminlight /*frad*/ - fdist ) * dl->color[1];
					pfBL[2] += ( fminlight /*frad*/ - fdist ) * dl->color[2];
					// jkrige - improved dynamic light falloff
				}
			}
		}
