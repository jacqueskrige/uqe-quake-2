...
// jkrige - cinematic rendering
void SCR_AllocCinematic (void)
{
	SCR_FreeCinematic ();

	// jkrige - doubled textures
	//if (cinframe == NULL)
	//	cinframe = Z_Malloc (cin.width * cin.height * 4);
	if (cinframe == NULL)
		cinframe = Z_Malloc((cin.width*2) * (cin.height*2) * 4);
	// jkrige - doubled textures
}
...
qboolean SCR_DrawCinematic (void)
{
	...
	// jkrige - cinematic rendering
	if (((float)viddef.width / (float)cin.width) < ((float)viddef.height / (float)cin.height))
	{
		cin_aspect = (float)viddef.width / (float)cin.width;
	}
	else
	{
		cin_aspect = (float)viddef.height / (float)cin.height;
	}
	cin_w = (int)((float)cin.width * cin_aspect);
	cin_h = (int)((float)cin.height * cin_aspect);

	re.DrawStretchRaw ((viddef.width-cin_w)/2, (viddef.height-cin_h)/2, cin_w, cin_h, cin.width, cin.height, cin.pic, cinframe);
...
#ifdef UQE_FMOD
	// make sure fmod isn't playing music
	FMOD_MusicStop();
#else
