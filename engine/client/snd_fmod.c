...
#pragma comment(lib, "./libraries/fmod-4/lib/fmodex_vc.lib")

#include "../libraries/fmod-4/inc/fmod.h"
#include "../libraries/fmod-4/inc/fmod_errors.h"
...
qboolean SND_FOpen (const char *name)
{
	...
	if ((len = FS_FOpenFile((char*)name, &f)) < 1)
	{
		Com_Printf("SND_FOpen: Failed to open %s, file not found\n", name);
		return false;
	}
	...
	if (!SND_File.length)
	{
		...
		SND_File.length = len;
		len = FS_LoadFile(SND_File.filename, (void **)&SND_File.data);
		Com_DPrintf("SND_FOpen: Sucessfully opened %s\n", name);

		return true;
	}

	Com_DPrintf("SND_FOpen: Failed to open %s, insufficient handles\n", name);
	...
	for (i = 0; i < numdrives; i++)
	{
		...
		if (fmod_result == FMOD_OK)
		{
			...
			if (fmod_result == FMOD_OK)
			{
				...
				if (fmod_result == FMOD_OK)
				{
					Com_Printf("CD Audio Initialized (%s)\n", drivename);
...
void FMOD_Startup (void)
{
	...
	switch (fmod_output)
	{
		case FMOD_OUTPUTTYPE_NOSOUND:
			Com_Printf("using No Sound\n");
			break;

		case FMOD_OUTPUTTYPE_DSOUND:
			Com_Printf("using Microsoft DirectSound\n");
			break;

		case FMOD_OUTPUTTYPE_WINMM:
			Com_Printf("using Windows Multimedia\n");
			break;

		case FMOD_OUTPUTTYPE_WASAPI:
			Com_Printf("using Windows Audio Session API\n");
			break;

		case FMOD_OUTPUTTYPE_ASIO:
			Com_Printf("using Low latency ASIO\n");
			break;
	}
	
	Com_Printf("   software channels: %i\n", SND_SoftwareChannels);
	Com_Printf("   hardware channels: %i\n", SND_HardwareChannels);
	Com_Printf("   %i bits/sample\n", SND_Bits);
	Com_Printf("   %i bytes/sec\n", SND_Rate);

	switch (speakermode)
	{
		case FMOD_SPEAKERMODE_RAW:
			Com_Printf("Speaker Output: Raw\n");
			break;

		case FMOD_SPEAKERMODE_MONO:
			Com_Printf("Speaker Output: Mono\n");
			break;

		case FMOD_SPEAKERMODE_STEREO:
			Com_Printf("Speaker Output: Stereo\n");
			break;

		case FMOD_SPEAKERMODE_QUAD:
			Com_Printf("Speaker Output: Quad\n");
			break;

		case FMOD_SPEAKERMODE_SURROUND:
			Com_Printf("Speaker Output: Surround\n");
			break;

		case FMOD_SPEAKERMODE_5POINT1:
			Com_Printf("Speaker Output: 5.1\n");
			break;

		case FMOD_SPEAKERMODE_7POINT1:
			Com_Printf("Speaker Output: 7.1\n");
			break;

		case FMOD_SPEAKERMODE_SRS5_1_MATRIX:
			Com_Printf("Speaker Output: Stereo compatible\n");
			break;

		case FMOD_SPEAKERMODE_MYEARS:
			Com_Printf("Speaker Output: Headphones\n");
			break;

		default:
			Com_Printf("Speaker Output: Unknown\n");
	}

	strcpy(bgmtype, "cd");
	oldmusicvolume = s_musicvolume->value;
	...
	SND_MusicChannel.loopcount = cd_loopcount->integer;
...
void FMOD_Shutdown(void)
{
	...
	Cmd_RemoveCommand("fmod_restart");
	Cmd_RemoveCommand("fmod_playmusic");
	Cmd_RemoveCommand("fmod_stopmusic");
	Cmd_RemoveCommand("fmod_pausemusic");
	Cmd_RemoveCommand("fmod_resumemusic");
...
void FMOD_ChannelStart (FMOD_SOUND *sound, qboolean loop, qboolean paused)
{
	...
	if ((SND_MusicChannel.looping = loop) == true)
	{
		...
		if (SND_MusicChannel.track != 0 &&
			SND_MusicChannel.track != cd_looptrack->integer)
		{
			fmod_result = FMOD_Channel_SetLoopCount(SND_MusicChannel.channel, cd_loopcount->integer);
			FMOD_ERROR(fmod_result, true, false);
		}
	}
	else
...
void FMOD_ChannelUpdate (void)
{
	...
	fmod_result = FMOD_Channel_GetLoopCount(SND_MusicChannel.channel, &SND_MusicChannel.loopcount);
	FMOD_ERROR(fmod_result, true, false);

	if (cd_loopcount->integer - SND_MusicChannel.loopcount >= cd_loopcount->integer)
		FMOD_MusicStart(cd_looptrack->string, true, false);
}
...
void CDA_Resume (qboolean force)
{
	...
	if (SND_MusicChannel.paused == true && SND_MusicChannel.volume != 0.0f && (oldmusicvolume == 0.0f | force == true))
	{
...
void CDA_Update (void)
{
	...
	SND_MusicChannel.volume = s_musicvolume->value;
...
void MOD_Start (char *name, qboolean loop, qboolean notify)
{
	...
	if (!fmod_sound)
	{
		Com_Printf("Couldn't open stream %s\n", file);
		return;
	}
	else
	{
...
void FMOD_MusicStart (char *name, qboolean loop, qboolean notify)
{
	...
	if (name != NULL && strlen(name) != 0)
	{
		...
		gamemode = Developer_searchpath(0);

		Com_sprintf(trackname, sizeof(trackname), "music/%s.ogg", file);
		len = FS_LoadFile(trackname, NULL);
		if (len < 0)
		{
			Com_sprintf(trackname, sizeof(trackname), "music/%s.mp3", file);
			len = FS_LoadFile(trackname, NULL);
		}
		if (len < 0)
		{
			Com_sprintf(trackname, sizeof(trackname), "music/%s.wav", file);
			len = FS_LoadFile(trackname, NULL);
		}
		if (len < 0)
		{
			Com_sprintf(trackname, sizeof(trackname), "music/%strack%02i.ogg", (gamemode != 0) ? va("mp%i_", gamemode) : "", atoi(file));
			len = FS_LoadFile(trackname, NULL);
		}
		if (len < 0)
		{
			Com_sprintf(trackname, sizeof(trackname), "music/%strack%02i.mp3", (gamemode != 0) ? va("mp%i_", gamemode) : "", atoi(file));
			len = FS_LoadFile(trackname, NULL);
		}
		if (len < 0)
		{
			Com_sprintf(trackname, sizeof(trackname), "music/%strack%02i.wav", (gamemode != 0) ? va("mp%i_", gamemode) : "", atoi(file));
			len = FS_LoadFile(trackname, NULL);
		}
	}
...
void FMOD_MusicStartConsole (void)
{
	...
	if (Cmd_Argc() < 2)
	{
		Com_Printf ("fmod_playmusic <filename> : play an audio file\n");
		return;
	}

	Com_sprintf(name, sizeof(name), "%s\0", Cmd_Argv(1));
	COM_StripExtension(name, file);
...
void FMOD_MusicUpdate (char *newbgmtype)
{
	...
	oldmusicvolume = SND_MusicChannel.volume;
}
