...
void SCR_DrawCrosshair (void)
{
	...
	re.DrawPic ((viddef.width/2) - ((crosshair_width*VID_SCALEFACTOR)/2), (viddef.height/2) - ((crosshair_height*VID_SCALEFACTOR)/2), crosshair_pic);
...
void V_RenderView( float stereo_separation )
{
	...
	if ( cl.frame.valid && (cl.force_refdef || !cl_paused->value) )
	{
		...
		//          640 = base width
		//          480 = base height
		//          90  = base fov (field of view)
		fov_monitor = ((float)cl.refdef.width / (float)cl.refdef.height) / (640.0f / 480.0f);
		fov_width = (2 * atan(fov_monitor * tan((90.0f / 2) * (M_PI / 180)))) * (180 / M_PI);
